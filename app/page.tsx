...
export default function Home() {
  ...
  async function retrieveSalary(match: OccupationMatch) {
    ...
    const structuredLocation = { city: locationParts.slice(0, -2).join(", "), region: locationParts.at(-2), country: locationParts.at(-1) };
    const response = await fetch("/api/salary/search", { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ jobTitle: job, occupation: match, location: structuredLocation, hoursPerWeek: number(hours) * number(days), weeksPerYear: number(weeks) }) });
  ...
  const usableSalary = (estimate: SalaryResult) => estimate.suggestedAnnual;
    ...
    <div className="layout">
      <div>
        ...
        <section className="window"><h2><span>2</span> Choose your target salary</h2>
          ...
          {occupationMatches.length > 0 && <fieldset className="occupation-list"><legend>We found these possible occupations. Which best matches your job?</legend>{occupationMatches.map((match) => <button type="button" className={selectedOccupation === match.code ? "selected" : ""} key={match.code} onClick={() => retrieveSalary(match)}><b>{match.title}</b><small>{match.classification} {match.code} · {Math.round(match.score * 100)}% title match</small></button>)}</fieldset>}
          {salaryResults.length > 0 && <div className="salary-data"><h3>Government salary data found</h3>{salaryResults.map((estimate, index) => { const annual = usableSalary(estimate); const label = estimate.salaryFieldSelected.startsWith("median") ? "Median" : estimate.salaryFieldSelected.startsWith("mean") ? "Mean (median unavailable)" : "High wage (median unavailable)"; return <article key={`${estimate.provider}-${index}`}><div><b>{estimate.provider}</b><strong>{formatCurrency(annual, estimate.currency)}</strong></div><p>{estimate.occupationTitle} ({estimate.classification} {estimate.occupationCode})<br/>{estimate.geographyName} · {estimate.dataYear || "Current data"}<br/>Base salary · {label}{estimate.annualizedFromHourly && ` · Annualized from ${formatCurrency(estimate.medianHourly || estimate.meanHourly || estimate.highHourly || 0, estimate.currency, 2)}/hour at ${formatHours(number(hours) * number(days))} hours/week × ${weeks} weeks`}{estimate.fallbackMessage && <><br/><b>{estimate.fallbackMessage}</b></>}</p>{estimate.sourceUrl && <a href={estimate.sourceUrl} target="_blank" rel="noreferrer">Official source</a>} <button type="button" onClick={() => { setTargets([String(Math.round(annual)), ...targets.slice(1)]); setCurrency(estimate.currency); }}>Use {formatCurrency(annual, estimate.currency)}</button></article>})}<button type="button" onClick={() => document.querySelector<HTMLInputElement>(".wide-label input")?.focus()}>Enter my own</button></div>}
          </form>
        </section>
